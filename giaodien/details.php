...
// Lấy id từ trang index.php truyền sang rồi hiển thị n`ó
if (isset($_GET['id'])) {
  $id = $_GET['id'];
...
<script>
    //Mã này la một tập lệnh JavaScript xác định một hàm để thay đổi thuộc tính nguồn (src) của một phần tử
    // hình ảnh HTML dựa trên hình ảnh nhỏ nào được nhấp vào.

    //Tập lệnh sử dụng các phương thức getElementById() và getElementsByClassName() để lấy tham chiếu đến
    // các phần tử hình ảnh HTML, sau đó xác định một số trình xử lý sự kiện onclick gán thuộc tính src của phần tử
    // MainImg cho thuộc tính src của hình ảnh nhỏ được nhấp
    // . Điều này tạo ra một bộ sưu tập hình ảnh cho phép người dùng chuyển đổi giữa các chế độ xem khác nhau của sản phẩm.

    //Mã HTML dường như bao gồm tên và giá sản phẩm, có thể được truy xuất từ ​​một biến PHP bằng cách

    // sử dụng các hàm echo và number_format. Tuy nhiên, đoạn mã được cung cấp không đầy đủ và không bao gồm cấu
    // trúc HTML đầy đủ hoặc bất kỳ mã CSS hoặc PHP đi kèm nào.
...
<div class="single-pro-details">
 ...
 <div class="about">
                ...
                <script>
                  ...
                  //Đây là một đoạn mã của trang chi tiết sản phẩm của một trang web thương mại điện tử.
                  // Tập lệnh đầu tiên thêm lớp "đang hoạt động" vào nút được nhấp trong một nhóm nút để đánh
                  // dấu nút đang hoạt động. Tập lệnh thứ hai có ba chức năng: updatePrice(), addToCart(), và buyNow().
                  // Chức updatePrice()năng cập nhật giá dựa trên số lượng do người dùng lựa chọn. Chức addToCart()năng
                  // thêm sản phẩm đã chọn vào giỏ hàng với số lượng xác định. Chức buyNow()năng thêm sản phẩm đã chọn
                  // vào giỏ hàng với số lượng là 1 và chuyển hướng người dùng đến trang thanh toán.
                  //
                  //Đánh dấu HTML bao gồm hình ảnh sản phẩm, tên, giá, bộ chọn số lượng và các nút để thêm vào
                  // giỏ hàng hoặc mua ngay. Các chi tiết sản phẩm được hiển thị trong một phần riêng biệt.

                  //Đánh dấu nút được chọn: Khi người dùng nhấp vào một nút trên thanh tiêu đề của trang, hàm updatePrice()
                  // được gọi để cập nhật giá trị cho sản phẩm dựa trên số lượng được chọn. Đồng thời, hàm addToCart()
                  // hoặc buyNow() được gọi để thêm sản phẩm vào giỏ hàng hoặc chuyển đến trang thanh toán ngay, tùy thuộc
                  // vào lựa chọn của người dùng. Trong cả hai trường hợp, giá trị id của sản phẩm được truyền vào hàm để
                  // xác định sản phẩm cụ thể được thêm vào giỏ hàng hoặc mua ngay.
                  //
                  //Hiển thị ảnh sản phẩm: Khi người dùng nhấp vào một ảnh nhỏ của sản phẩm, hàm updateImage() được
                  // gọi để cập nhật ảnh lớn của sản phẩm với ảnh được chọn.
                  //
                  //Cập nhật giá sản phẩm: Hàm updatePrice() được gọi để cập nhật giá trị của sản phẩm dựa trên số
                  // lượng sản phẩm được chọn. Hàm này lấy giá sản phẩm từ phần tử có id là price, số lượng sản phẩm
                  // từ phần tử có id là num, và giá tiền từ phần tử có class là gia. Sau đó, hàm tính toán tổng giá
                  // trị và cập nhật giá trị cho phần tử có id là price.
                  //
                  //Thêm sản phẩm vào giỏ hàng: Hàm addToCart() được gọi khi người dùng nhấp vào nút "Thêm vào giỏ
                  // hàng". Hàm này gửi một yêu cầu POST đến tệp cookie.php với các tham số action, id, và num.
                  // Tham số action được sử dụng để xác định hành động cần thực hiện, tham số id xác định sản phẩm
                  // được thêm vào giỏ hàng, và tham số num xác định số lượng sản phẩm được thêm vào giỏ hàng.
                  //
                  //Mua ngay: Hàm buyNow() được gọi khi người dùng nhấp vào nút "Mua ngay". Hàm này tương tự như
                  // addToCart(), nhưng số lượng sản phẩm được mua ngay luôn là 1. Sau khi sản phẩm được thêm vào giỏ
                  // hàng, trình duyệt sẽ chuyển hướng đến trang thanh toán ngay.
                  //
                  //Các phần tử HTML trên trang web được chọn bằng các phương thức getElementById(), getElementsByClassName(),
                  // và querySelector(). Các sự kiện như "click"
    ...
    <span>
      ...
      <section class="restaurants container">
        ...
        <div class="product-restaurants">
          <div class="row">
            <?php
            $sql = 'select * from product,report where type_clothes ="ao"  AND product.id=report.id_report';
...
//getElementById(): Phương thức này sử dụng để lấy đối tượng HTML bằng cách sử dụng id của nó.
Trong đoạn mã của bạn, đối tượng HTML được lấy bằng cách sử dụng phương thức này là "MainImg".
//
//getElementsByClassName(): Phương thức này được sử dụng để lấy danh sách các đối tượng HTML dựa trên tên lớp của chúng.
Trong đoạn mã của bạn, danh sách các đối tượng HTML được lấy bằng cách sử dụng phương thức này là "small-img".
//
//addEventListener(): Phương thức này được sử dụng để gắn kết một hàm xử lý sự kiện với một đối tượng HTML.
Trong đoạn mã của bạn, hàm xử lý sự kiện được gắn kết với các nút "btn" trong phần "myDIV".
//
//replace(): Phương thức này được sử dụng để thay thế một chuỗi ký tự bằng một chuỗi ký tự khác.
Trong đoạn mã của bạn, phương thức này được sử dụng để thay thế chuỗi "active" bằng chuỗi rỗng "".
//
//toLocaleString(): Phương thức này được sử dụng để chuyển đổi một số thành một chuỗi dạng địa phương.
Trong đoạn mã của bạn, phương thức này được sử dụng để định dạng lại giá trị tổng giá tiền của sản phẩm theo định dạng địa phương.
//
//querySelector(): Phương thức này được sử dụng để lấy đối tượng HTML đầu tiên mà nó tìm thấy trong tài liệu,
dựa trên một bộ chọn CSS. Trong đoạn mã của bạn, phương thức này được sử dụng để lấy đối tượng HTML của phần tử "gia".
//
//post(): Phương thức này được sử dụng để gửi yêu cầu HTTP POST đến một trang web và xử lý phản hồi
. Trong đoạn mã của bạn, phương thức này được sử dụng để thêm sản phẩm vào giỏ hàng hoặc mua ngay.
</body>
<style>
   /* single product */
        #prodetails {
